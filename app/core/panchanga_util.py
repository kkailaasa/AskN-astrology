...
gregorian_to_jd = lambda date: swe.julday(
    date.year,
    date.month,
    date.day,
    # 0.0
...
def sunrise(jd, place):
    ...
    result = swe.rise_trans(
        jd - tz / 24, swe.SUN, swe.BIT_DISC_CENTER + swe.CALC_RISE, (lon, lat, 0.0)
    )
    rise = result[1][0]  # julian-day number
    # Convert to local time
    return [rise + tz / 24.0, to_dms((rise - jd) * 24 + tz)]
...
def sunset(jd, place):
    ...
    lat, lon, tz = place
    result = swe.rise_trans(
        jd - tz / 24, swe.SUN, swe.BIT_DISC_CENTER + swe.CALC_SET, (lon, lat, 0.0)
    )
    setting = result[1][0]  # julian-day number
    # Convert to local time
    return [setting + tz / 24.0, to_dms((setting - jd) * 24 + tz)]
...
def moonrise(jd, place):
    ...
    lat, lon, tz = place
    result = swe.rise_trans(
        jd - tz / 24, swe.MOON, lon, lat, rsmi=swe.BIT_DISC_CENTER + swe.CALC_RISE
    )
    rise = result[1][0]  # julian-day number
    # Convert to local time
...
def moonset(jd, place):
    ...
    lat, lon, tz = place
    result = swe.rise_trans(
        jd - tz / 24, swe.MOON, lon, lat, rsmi=swe.BIT_DISC_CENTER + swe.CALC_SET
    )
    setting = result[1][0]  # julian-day number
    # Convert to local time
...
def tithi(jd, place, jd_for_srise):
    ...
    lunar_long_diff = [
        (lunar_longitude(rise + t) - lunar_longitude(rise)) % 360 for t in offsets
    ]
    solar_long_diff = [
        (solar_longitude(rise + t) - solar_longitude(rise)) % 360 for t in offsets
    ]
    relative_motion = [
...
def nakshatra(jd, place, jd_for_sunrise):
    ...
    # 1. Find time of sunrise
    lat, lon, tz = place
    rise = sunrise(jd_for_sunrise, place)[0] - tz / 24.0  # Sunrise at UT 00:00

    # Swiss Ephemeris always gives Sayana. So subtract ayanamsa to get Nirayana
    offsets = [0.0, 0.25, 0.5, 0.75, 1.0]
    longitudes = [
        (lunar_longitude(rise + t) - swe.get_ayanamsa_ut(rise)) % 360 for t in offsets
    ]
...
def yoga(jd, place, jd_for_sunrise):
